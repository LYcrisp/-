...
class Coach:
    def __init__(self, handler):
        ...
        # ----------------new---------------------
        self.item_popularity = handler.item_popularity

        # 构建 Item-Item 共现图
        self.item_cooc = self.buildItemItemGraph(handler.trnMat)  # 新增的共现图构建
        self.handler.kg_edges = self.addCooccurrenceToKG(self.handler.kg_edges, self.item_cooc)  # 将共现关系作为边加入到KG中
    ...
    def buildItemItemGraph(self, trnMat):
        """
        基于训练集的交互矩阵构建物品-物品共现图。
        """
        item_cooc = trnMat.T.dot(trnMat).tocoo()  # 计算物品-物品共现矩阵
        return item_cooc

    def addCooccurrenceToKG(self, kg_edges, item_cooc):
        """
        将计算的Item-Item共现关系作为附加边加入到知识图谱。
        """
        new_edges = []
        for row, col, value in zip(item_cooc.row, item_cooc.col, item_cooc.data):
            if row != col:  # 排除物品自己与自己的共现
                # 添加三元组到知识图谱边列表：物品A → 物品B, 关系=共现
                new_edges.append([row, col, 'cooccurrence'])
                new_edges.append([col, row, 'cooccurrence'])  # 反向边

        # 将新的共现边添加到现有的知识图谱
        kg_edges.extend(new_edges)
        return kg_edges
    ...
    def trainEpoch(self):
        ...
        for i, batch in enumerate(diffusionLoader):
            ...
            diff_loss, ukgc_loss = self.diffusion_model.training_losses(self.denoise_model, batch_item, ui_matrix,
                                                                        uEmbeds, iEmbeds, batch_index)
        ...
        for i, tem in enumerate(trnLoader):
            ...
            # ------------new------------------------
            # 获取负样本的流行度，形状为 (batch,)
            pop_penalty = self.item_popularity[negs]

            # 加入惩罚项（越热门，惩罚越大）
            scoreDiff = pairPredict(ancEmbeds, posEmbeds, negEmbeds)
            scoreDiff = scoreDiff - args.pop_alpha * pop_penalty  # 加权调节

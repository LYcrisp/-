...
class Hope():
    def __init__(self, args, data, distanceMat, itemMat, categoryMat, item_features):
        self.args = args
        ...
        self.categoryMat = categoryMat  # 保存物品类别矩阵
        self.item_features = item_features  # 保存物品特征数量（即类别总数）
    ...
    def prepareModel(self):
        ...
        self.model = MODEL(
            ...
            self.args.hide_dim,
            self.args.Layers,
            item_features=self.item_features).cuda()
    ...
    def trainModel(self):
        ...
        for user, item_i, item_j in self.train_loader:
            ...
            # 获取物品类别信息作为内容嵌入输入
            item_content = t.tensor(self.categoryMat[itemindex.cpu().numpy()], dtype=t.long).cuda()

            # 模型正向传播（结构+内容融合）
            self.userEmbed, self.itemEmbed, self.ui_userEmbedall, self.ui_itemEmbedall, \
                self.ui_userEmbed, self.ui_itemEmbed, metaregloss = self.model(
                self.train, userindex, itemindex, item_content=item_content, norm=1)

            # 内容对比损失：结构表示 vs 内容表示
            content_embed = self.model.item_content_embedding(item_content)
            content_contrastive_loss = self.model.content_contrastive_loss(self.itemEmbed[itemindex], content_embed)
            ...
            pred_pos, pred_neg = self.predictModel(
                self.ui_userEmbedall[user], self.ui_itemEmbedall[item_i], self.ui_itemEmbedall[item_j])
            ...
            loss = (
                    (bpr_loss + self.args.reg * regLoss) / self.args.batch
                    + ssl_loss * self.args.ssl_beta
                    + metaregloss * self.args.metareg
                    + content_contrastive_loss * self.args.ssl_beta  # 加入内容对比损失
            )
    ...
    def testModel(self):
        ...
        with t.no_grad():
            ...
            # 获取所有 item 的内容信息（类别向量）
            item_content = t.tensor(self.categoryMat[iid], dtype=t.long).cuda()

            # 将 item_content 传入 forward
            _, _, self.ui_userEmbed, self.ui_itemEmbed, _, _, _ = self.model(
                self.train, uid, iid, item_content=item_content, norm=1
            )
...
if __name__ == '__main__':
    ...
    categoryMat = data[3].toarray().reshape(-1)  # 获取物品类别信息
    item_features = int(np.max(categoryMat)) + 1  # 类别总数

    # model instance
    # hope = Hope(args, data, distanceMat, itemMat)
    hope = Hope(args, data, distanceMat, itemMat, categoryMat, item_features)
